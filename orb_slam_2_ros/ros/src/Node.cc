...
Node::Node (ORB_SLAM2::System* pSLAM, ros::NodeHandle &node_handle, image_transport::ImageTransport &image_transport) {
  ...
  // 改 ,,此部分发布的数据飘移很大
  node_handle_.param(name_of_node_+"/publish_odometry", publish_odom_param_, true);
  ...
  if (publish_odom_param_) {
    odom_publisher_ = node_handle_.advertise<nav_msgs::Odometry> (name_of_node_+"/vo", 1);
  }
...
// 从system.cc里返回GetCurrentPosition（current_position_ = Tcw） 赋值给position
void Node::Update () {
...
//  cout << "position is " << position << endl;
  ...
  if (!position.empty()) {
    ...
    // 发布VO
    if(publish_odom_param_){
        pose = PublishPositionAsPoseStamped (position);
        PublishOdom(pose);
    }
  }
...
// Mat转MatrixXD ,这个转换没有问题
Eigen::Matrix3d Node::toEigenMatrix3d(const cv::Mat &cvMat)
{
...
// 发布vo
void Node::PublishOdom(geometry_msgs::PoseStamped pose_msg)
{
    nav_msgs::Odometry odom;
    std_msgs::Header header;
    odom.header = header;
    odom.header.stamp = current_frame_time_;
...
// 发布pose
geometry_msgs::PoseStamped Node::PublishPositionAsPoseStamped (cv::Mat position) {
  tf::Transform grasp_tf = TransformFromMat (position); // pose转换

  tf::Stamped<tf::Pose> grasp_tf_pose(grasp_tf, current_frame_time_, map_frame_id_param_);
  geometry_msgs::PoseStamped pose_msg;
  tf::poseStampedTFToMsg (grasp_tf_pose, pose_msg);
  pose_publisher_.publish(pose_msg);  // pose_msg包括x y z，qx qy qz qw
  return pose_msg;
...
tf::Transform Node::TransformFromMat (cv::Mat position_mat) {
  ...
  return tf::Transform (tf_camera_rotation, tf_camera_translation); // 返回相机的旋转平移矩阵
}
