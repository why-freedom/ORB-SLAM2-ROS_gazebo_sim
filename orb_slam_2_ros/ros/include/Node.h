...
class Node
{
  ...
  private:
	...
	void PublishOdom(geometry_msgs::PoseStamped pose_msg); // 发布里程计信息	
    geometry_msgs::PoseStamped PublishPositionAsPoseStamped(cv::Mat position); // 发布位姿
    void PublishRenderedImage (cv::Mat image);
    void ParamsChangedCallback(orb_slam2_ros::dynamic_reconfigureConfig &config, uint32_t level);
    tf::Transform TransformFromMat (cv::Mat position_mat);
	Eigen::Matrix3d toEigenMatrix3d(const cv::Mat &cvMat);
